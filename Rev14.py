# Rev14.py - FULLY RESTORED AND CORRECTED CODE
...
# ## FIX ##: Removed the faulty 'from_json' import
...
# ## IMPROVEMENT 5 ##: Added config for better news quality control
app.config['NEWS_API_SOURCES'] = os.environ.get('NEWS_API_SOURCES', '') # e.g., 'techcrunch,the-verge,wired'
...
if GROQ_API_KEY:
    try:
        # Using a powerful model for better JSON compliance and summary quality
...
# ## FIX 1 & 2 ##: More robust AI summary/takeaway generation
...
# ## IMPROVEMENT 5 ##: Higher quality news fetching
@simple_cache()
...
def inject_global_vars():
    # Pass user info via session object which is already available in Jinja
...
def index(page=1, category_name='All Articles'):
    ...
    # Restored featured article logic from your original code
...
def search_results(page=1):
    ...
    all_search_results = sorted(api_results + community_results, 
                                key=lambda x: getattr(x, 'published_at', datetime.fromisoformat(x.get('publishedAt', '1970-01-01T00:00:00Z').replace('Z', ''))) if isinstance(x, dict) else x.published_at, 
                                reverse=True)
...
# ## FIX 3 ##: Simplified and robust article routing
...
def article_detail(article_hash_id):
    ...
    # First, check if it's a community article by its unique hash_id
    ...
    if article_db:
        ...
        # ## FIX ##: Parse takeaways JSON here, before sending to template
        ...
        # If not, assume it's an API article from the in-memory store
    ...
    # This logic ensures the template can consistently access the flag.
    if isinstance(article_data, dict):
        article_data['is_community_article'] = False
    else: # It's a DB object
...
def add_comment(article_hash_id):
    ...
    # Check if the comment is for a community article or an API article
...
def post_article():
    ...
    if groq_analysis_result and not groq_analysis_result.get("error"):
        ...
        groq_takeaways_json = json.dumps(groq_analysis_result.get('groq_takeaways'))
    ...
    # ## FIX 3 ##: Redirect to the correct article detail page using the new hash
...
# ==============================================================================
# --- 7. HTML Templates (Restored to your original versions with fixes) ---
# ==============================================================================
BASE_HTML_TEMPLATE = """
<!doctype html>
<html lang="en">
...
{% endblock %}
{% block content %}
    {# Featured Article Section - Restored from your original template #}
    ...
    <article class="featured-article p-md-4 p-3 mb-4 animate-fade-in">
        <div class="row g-0 g-md-4">
            {% set art0 = articles[0] %}
            {# ## FIX 3 ##: The URL now correctly uses the hash_id for community articles and id for API articles #}
...
<article class="article-full-content-wrapper animate-fade-in">
    ...
    <div class="article-meta-detailed">
        ...
        <span class="meta-item" title="Published Date"><i class="far fa-calendar-alt"></i> {{ (article.published_at.strftime('%Y-%m-%d') if is_community_article else (article.publishedAt.split('T')[0] if article.publishedAt else 'N/A')) }}</span>
    ...
    {% if is_community_article %}
        ...
        {# ## FIX ##: This now uses the 'parsed_takeaways' list prepared in the Python route #}
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (isCommunityArticle) {
        // For community articles, calculate read time from the full_text passed from backend
        const fullText = {{ article.full_text | tojson if is_community_article else 'null' }};
        calculateAndSetReadTime(fullText);
    } else if (articleHashId) {
        // For API articles, fetch content and then calculate
    ...
    if (commentForm && articleHashId) {
        commentForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if(data.success) {
                    // Dynamically add the new comment without reloading the page
