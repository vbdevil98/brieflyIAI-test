...
# ==============================================================================
# In Rev14.py, REPLACE the entire BASE_HTML_TEMPLATE variable with this definitive version.
...
<head>
    ...
    <style>
        ...
        .alert-top { position: fixed; top: 20px; left: 50%; transform: translateX(-50%); z-index: 2050; text-align:center; box-shadow: var(--shadow-lg); }
        .navbar-main { background-color: var(--primary-color); padding: 0.5rem 0; box-shadow: var(--shadow-md); z-index: 1031; height: 70px; }
        .navbar-brand-custom { color: white !important; font-weight: 700; font-size: 2rem; }
        .navbar-brand-custom .brand-icon { color: var(--secondary-light); }
        .desktop-nav-controls { gap: 1rem; }
        .search-container { position: relative; width: 100%; max-width: 550px; }
        .navbar-search { width: 100%; border-radius: 50px; padding: 0.6rem 1.25rem 0.6rem 2.8rem; border: 1px solid transparent; background: rgba(255,255,255,0.15); color: white; }
        .navbar-search:focus { background: rgba(255,255,255,0.25); box-shadow: 0 0 0 4px rgba(255,255,255,0.2); }
        .search-icon { position: absolute; left: 1.1rem; top: 50%; transform: translateY(-50%); color: rgba(255,255,255,0.8); }
        .header-btn { background: transparent; border: 1px solid rgba(255,255,255,0.4); padding: 0.5rem 1rem; border-radius: 50px; color: white; transition: all 0.3s ease; }
        .header-btn:hover { background: rgba(255,255,255,0.9); color: var(--primary-dark); }
        .dark-mode-toggle { font-size: 1.1rem; width: 42px; height: 42px; display: flex; align-items: center; justify-content: center;}
        
        .category-nav { background: var(--card-bg); position: fixed; top: 70px; width: 100%; z-index: 1030; border-bottom: 1px solid var(--card-border-color); height: 60px; display:flex; align-items:center; }
        .category-links-container { display: flex; flex-shrink: 0; }
        .category-link { color: var(--text-muted-color) !important; font-weight: 600; padding: 0.5rem 1.2rem !important; border-radius: 50px; white-space: nowrap; text-decoration: none; margin: 0 0.3rem; }
        .category-link.active { background: var(--primary-color) !important; color: white !important; }
        
        .mobile-nav-toggle { background: none; border: none; color: white; font-size: 1.8rem; line-height: 1; }
        .offcanvas.offcanvas-end { background-color: var(--card-bg); }
        .offcanvas-body .nav { flex-direction: column; }
        .offcanvas-body .nav-link { color: var(--text-color); font-weight: 500; padding: 0.75rem 0; font-size: 1.1rem; }
        .offcanvas-body .nav-link:hover { color: var(--primary-color); }
        .offcanvas-body .nav-link i { width: 25px; text-align: center; margin-right: 0.75rem; color: var(--text-muted-color); }
        
        .ai-synthesis-card { border-radius: var(--border-radius-lg); padding: 2.5rem; margin-bottom: 2.5rem; box-shadow: var(--shadow-lg); position: relative; overflow: hidden; border: 1px solid var(--card-border-color); }
        .ai-synthesis-card::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-image: linear-gradient(135deg, rgba(var(--primary-color-rgb), 0.05) 25%, transparent 25%), linear-gradient(225deg, rgba(var(--primary-color-rgb), 0.05) 25%, transparent 25%); background-size: 20px 20px; opacity: 0.5; z-index: 0; }
        .ai-synthesis-card > * { position: relative; z-index: 1; }
        .synthesis-header i { font-size: 2rem; color: var(--primary-color); }
        .synthesis-text { font-size: 1.15rem; line-height: 1.7; }
        ...
        .admin-controls { position: fixed; bottom: 25px; right: 25px; z-index: 1029; }
        ...
        */
        @media (max-width: 991.98px) {
            body {
                padding-top: 70px; /* On mobile, only account for the single top navbar */
            }
            ...
            .main-content h4.fst-italic {
                margin-top: 1rem; /* Add space for search/category titles that might get too close to the nav */
            ...
            .featured-story-content, .ai-synthesis-card {
                padding: 1.5rem;
            }
            .featured-story-content h2, .synthesis-header h2 {
                font-size: 1.5rem;
            }
            .synthesis-text {
                font-size: 1.05rem;
...
<body class="{{ request.cookies.get('darkMode', 'disabled') }} {% block body_class %}{% endblock %}">
    ...
    <header class="fixed-top">
        <nav class="navbar-main">
            <div class="container d-flex justify-content-between align-items-center">
                ...
                <div class="d-none d-lg-flex align-items-center desktop-nav-controls">
    ...
    <div class="offcanvas offcanvas-end d-lg-none" tabindex="-1" id="mobileOffcanvas" aria-labelledby="mobileOffcanvasLabel">
        ...
        <div class="offcanvas-body d-flex flex-column">
            <form action="{{ url_for('search_results') }}" method="GET" class="search-container mb-3">
                <input type="search" name="query" class="form-control navbar-search" placeholder="Search news..." value="{{ request.args.get('query', '') }}">
            </form>
            <hr>
            <nav class="nav">
            ...
            <div class="mt-auto">
                ...
                <div class="text-center mt-3">
                     <button class="btn dark-mode-toggle fs-4"><i class="fas fa-moon"></i></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="addArticleModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header"><h5 class="modal-title">Post New Article</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
                <div class="modal-body">
                    <form id="addArticleForm" action="{{ url_for('post_article') }}" method="POST">
                        ...
                        <div class="mb-3"><label class="form-label">Source</label><input type="text" name="sourceName" class="form-control" value="Community Post" required></div>
                        <div class="mb-3"><label class="form-label">Image URL</label><input type="url" name="imageUrl" class="form-control"></div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button><button type="submit" class="btn btn-primary">Post</button></div>
                    </form>
                </div>
            </div>
        </div>
    ...
    <footer class="mt-auto footer bg-dark text-light py-5">
       <div class="container">
           <div class="row">
               <div class="col-lg-4 col-md-6 mb-4">
                    <h5 class="text-white">Briefly</h5>
           ...
           <div class="text-center pt-3 mt-3 border-top border-secondary">&copy; {{ current_year }} Briefly. All rights reserved.</div>
       </div>
    </footer>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // Global Bookmark Handler (Restored and Verified)
        ...
        document.body.addEventListener('click', function(event) {
            ...
            .then(({ ok, json }) => {
                ...
                if (json.success) {
                    document.querySelectorAll(`.bookmark-btn[data-article-hash-id="${data.articleHashId}"]`).forEach(btn => {
                        btn.classList.toggle('active', json.status === 'added');
                        btn.title = json.status === 'added' ? 'Remove Bookmark' : 'Add Bookmark';
                    });
                }
            })
