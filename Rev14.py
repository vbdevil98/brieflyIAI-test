...
class User(db.Model):
    ...
    is_admin = db.Column(db.Boolean, nullable=False, default=False) # <-- NEW: Admin flag
    ...
    # Back-reference for reports filed by the user
    filed_reports = db.relationship('ReportedArticle', backref='reporter', lazy='dynamic', foreign_keys='ReportedArticle.reporter_user_id', cascade="all, delete-orphan")
...
class ReportedArticle(db.Model):
    ...
    # The 'reporter' relationship is established via the `backref` on the `User` model.
...
# NEW: Decorator for admin-only routes
def admin_required(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        if not session.get('is_admin'):
            # For JSON requests, return a JSON error
            if request.headers.get('Accept') == 'application/json':
                return jsonify({"success": False, "error": "Administrator access required."}), 403
            # For page loads, flash a message and redirect
            else:
    ...
    if isinstance(date_val, str):
        try:
            if date_val.endswith('Z'):
                date_val_dt = datetime.fromisoformat(date_val[:-1] + '+00:00')
            elif '+' in date_val or '-' in date_val[10:]:
                date_val_dt = datetime.fromisoformat(date_val)
            else:
                date_val_dt = datetime.fromisoformat(date_val).replace(tzinfo=timezone.utc)
            return date_val_dt
        except ValueError:
...
def vote_comment(comment_id):
    ...
    reaction_counts_query = db.session.query(
        CommentVote.vote_emoji, func.count(CommentVote.vote_emoji)
...
def delete_comment(comment_id):
    comment = Comment.query.options(joinedload(Comment.replies)).get_or_404(comment_id)

    # UPDATED: Allow deletion if user is the owner OR an admin.
    ...
    try:
        ...
        db.session.commit()
        app.logger.info(f"User {session['user_id']} (Admin: {session.get('is_admin', False)}) deleted comment {comment_id} and its replies.")
...
def report_article(article_hash_id):
    ...
    try:
        ...
        return jsonify({"success": True, "message": "Article has been reported for review. Thank you."})
    except Exception as e:
        db.session.rollback()
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, password):
            ...
            session['is_admin'] = user.is_admin # <-- NEW: Set admin status in session
...
# UPDATED: profile() route now includes Admin Dashboard functionality
@app.route('/profile')
@login_required
def profile():
    ...
    # NEW: Fetch reported articles if the user is an admin
    reported_articles = None
    if user.is_admin:
        reported_articles = ReportedArticle.query.options(
            joinedload(ReportedArticle.article).joinedload(CommunityArticle.author),
            joinedload(ReportedArticle.reporter)
        ).filter(ReportedArticle.status == 'pending').order_by(ReportedArticle.timestamp.desc()).all()
    ...
    return render_template(
        ...
        bookmarks_pagination=user_bookmarks_paginated_query, 
        current_page=page,
        reported_articles=reported_articles  # Pass reported articles to the template
    )
...
def admin_delete_article(article_hash_id):
    ...
    try:
        # Cascading deletes will handle comments and reports on this article
        db.session.delete(article)
        db.session.commit()
        flash(f"Article '{article.title}' has been permanently deleted.", "success")
        app.logger.info(f"Admin {session['user_name']} deleted article {article.id} ({article.title})")
        return jsonify({"success": True})
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Admin error deleting article {article.id}: {e}", exc_info=True)
        return jsonify({"success": False, "error": "A database error occurred."}), 500
...
def admin_dismiss_report(report_id):
    report = ReportedArticle.query.get(report_id)
    if not report:
    ...
    try:
        ...
        app.logger.info(f"Admin {session['user_name']} dismissed report {report.id}")
        return jsonify({"success": True})
    except Exception as e:
        db.session.rollback()
        app.logger.error(f"Admin error dismissing report {report.id}: {e}", exc_info=True)
        return jsonify({"success": False, "error": "A database error occurred."}), 500
...
<head>
    ...
    <style>
        :root {
            ...
            --admin-badge-bg: var(--accent-color); --admin-badge-text: white;
        }
        ...
        .admin-badge { background-color: var(--admin-badge-bg); color: var(--admin-badge-text); font-size: 0.7rem; padding: 0.2em 0.5em; border-radius: 0.25rem; font-weight: 700; vertical-align: middle; }
        ...
        /* Other styles from original template... */
        ...
        .featured-story, .ai-synthesis-card { display: block; }
        .featured-story { background-color: var(--card-bg); border-radius: var(--border-radius-lg); box-shadow: var(--shadow-lg); margin-bottom: 2.5rem; overflow: hidden; border: 1px solid var(--card-border-color); }
        ...
        .ai-synthesis-card { background: var(--card-bg); border: 1px solid var(--card-border-color); border-radius: var(--border-radius-lg); padding: 2rem; margin-bottom: 2.5rem; box-shadow: var(--shadow-lg); position: relative; overflow: hidden; }
        .ai-synthesis-card::before { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 100%; background-image: linear-gradient(135deg, rgba(var(--primary-color-rgb), 0.05) 25%, transparent 25%), linear-gradient(225deg, rgba(var(--primary-color-rgb), 0.05) 25%, transparent 25%); background-size: 20px 20px; opacity: 0.5; }
        ...
        .synthesis-header h2 { font-size: 1.5rem; margin-top: 0.5rem; }
        .synthesis-text { font-size: 1.15rem; line-height: 1.7; text-align: center; color: var(--text-color); position: relative; font-family: 'Inter', serif; }
        ...
        .synthesis-keywords .keyword-tag { display: inline-block; background-color: var(--light-bg); border: 1px solid var(--card-border-color); color: var(--text-muted-color); padding: 0.4rem 1rem; border-radius: 50px; margin: 0.25rem; font-size: 0.9rem; font-weight: 500; text-decoration: none; transition: all 0.2s ease-in-out; }
        ...
        @media (max-width: 991.98px) {
            ...
            .featured-story-content h2 { font-size: 2rem; }
        }
        @media (max-width: 767.98px) {
            ...
            .featured-story-content h2 { font-size: 1.6rem; }
            .footer-section, .footer-links, .social-links { text-align: center; justify-content: center; align-items: center; }
...
<body class="{{ request.cookies.get('darkMode', 'disabled') }}{% block body_class %}{% endblock %}">
                ...
                {% if session.user_id %}
                    <div class="dropdown">
                        <a href="#" class="sidebar-btn dropdown-toggle" id="offcanvasUserDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                            ...
                            <strong class="ms-3">{{ session.user_name|truncate(20) }}
                                {# NEW: Admin badge in menu #}
                                {% if session.get('is_admin') %}<span class="badge bg-light text-dark ms-2 small">Admin</span>{% endif %}
                            </strong>
                        </a>
                        <ul class="dropdown-menu shadow" aria-labelledby="offcanvasUserDropdown">
                            ...
                            {# NEW: Link to Admin Panel #}
                            {% if session.get('is_admin') %}
                            <li><a class="dropdown-item" href="{{ url_for('profile') }}#admin-panel-tab-pane"><i class="fas fa-shield-alt fa-fw me-2"></i>Admin Panel</a></li>
                            {% endif %}
    ...
    <footer class="mt-auto">
        <div class="container">
            ...
            <div class="copyright">&copy; """ + str(datetime.now().year) + """ BrieflyAI. All rights reserved. Made with <i class="fas fa-heart text-danger"></i> in India.</div>
        </div>
    </footer>
...
# UPDATED: _COMMENT_TEMPLATE with Admin Badge and Controls
_COMMENT_TEMPLATE = """
<div class="comment-thread" id="comment-{{ comment.id }}">
    <div class="comment-container">
        ...
        <div class="comment-body">
            <div class="comment-header">
                ...
                {# NEW: Admin badge for comments #}
                {% if comment.author and comment.author.is_admin %}
            ...
            <div class="comment-actions">
                ...
                {# UPDATED: Show edit button only to owner #}
                {% if session.user_id == comment.user_id %}
                    <button class="edit-btn" data-comment-id="{{ comment.id }}" title="Edit"><i class="fas fa-pencil-alt"></i> Edit</button>
                {% endif %}
                {# UPDATED: Show delete button to owner OR admin #}
                {% if session.user_id == comment.user_id or session.get('is_admin') %}
            ...
            <div class="reply-form-container" id="reply-form-container-{{ comment.id }}" style="display:none;">
                <form class="reply-form">
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        <button type="button" class="btn btn-sm btn-outline-secondary cancel-reply-btn" onclick="this.closest('.reply-form-container').style.display='none';">Cancel</button>
            ...
            <div class="reaction-summary" id="reaction-summary-{{ comment.id }}">
                {# Reaction pills will be dynamically inserted here by JS from article_detail template #}
                {% if comment_data and comment_data[comment.id] %}
                    {% for emoji, count in comment_data[comment.id].reactions.items() %}
                        <div class="reaction-pill {% if comment_data[comment.id].user_reaction == emoji %}user-reacted{% endif %}" data-emoji="{{ emoji }}">
    ...
    <div class="comment-replies" id="replies-of-{{ comment.id }}">
        {% for reply in comment.replies %}
            {% include '_COMMENT_TEMPLATE' with comment=reply, comment_data=comment_data %}
        {% endfor %}
    </div>
</div>
"""

# UPDATED: PROFILE_HTML_TEMPLATE with Admin Dashboard Panel
PROFILE_HTML_TEMPLATE = """
...
<div class="profile-header-card animate-fade-in">
    ...
    <h2>{{ user.name }} 
        {# NEW: Admin badge on profile page #}
...
<div class="mt-4 animate-fade-in" style="animation-delay: 0.1s;">
    <ul class="nav nav-tabs profile-tabs nav-fill mb-4" id="profileTab" role="tablist">
        {# NEW: Add Admin Panel tab if user is admin #}
        {% if user.is_admin and reported_articles is not none %}
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="admin-panel-tab" data-bs-toggle="tab" data-bs-target="#admin-panel-tab-pane" type="button" role="tab" aria-controls="admin-panel-tab-pane" aria-selected="false">
                <i class="fas fa-shield-alt me-2"></i>Admin Panel <span class="badge rounded-pill bg-danger ms-1">{{ reported_articles|length }}</span>
            </button>
        </li>
        {% endif %}
        <li class="nav-item" role="presentation">
    ...
    <div class="tab-content" id="profileTabContent">
        ...
        {% if user.is_admin and reported_articles is not none %}
        <div class="tab-pane fade" id="admin-panel-tab-pane" role="tabpanel" aria-labelledby="admin-panel-tab" tabindex="0">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Pending Reported Articles</h5>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover mb-0">
                        ...
                        <tbody id="reported-articles-tbody">
                        {% for report in reported_articles %}
                            <tr id="report-row-{{ report.id }}">
                                <td><a href="{{ url_for('article_detail', article_hash_id=report.article.article_hash_id) }}" target="_blank">{{ report.article.title|truncate(50) }}</a></td>
                                <td><a href="{{ url_for('public_profile', username=report.article.author.username) }}">{{ report.article.author.name }}</a></td>
                                <td><a href="{{ url_for('public_profile', username=report.reporter.username) }}">{{ report.reporter.name }}</a></td>
                                <td>{{ report.timestamp | to_ist }}</td>
                                <td>
                                    <div class="btn-group btn-group-sm" role="group">
                                        <button class="btn btn-outline-secondary dismiss-report-btn" data-report-id="{{ report.id }}" title="Dismiss Report"><i class="fas fa-check"></i></button>
                                        <button class="btn btn-outline-danger delete-article-btn" data-article-hash-id="{{ report.article.article_hash_id }}" data-report-id="{{ report.id }}" title="Delete Article"><i class="fas fa-trash"></i></button>
                                    </div>
                                </td>
                            </tr>
                        {% endfor %}
                ...
                <div class="card-body text-center">
                    <p class="mb-0 text-muted"><i class="fas fa-check-circle text-success me-2"></i>No pending reports. Great job!</p>
                </div>
                {% endif %}
            </div>
        </div>
            ...
            <div class="row g-4">
                ...
                <div class="col-md-6 col-lg-4 d-flex">
                    <article class="article-card d-flex flex-column w-100">
                        <div class="article-image-container"><a href="{{ art.article_url }}"><img src="{{ art.urlToImage or 'https://via.placeholder.com/400x220/EEEEEE/AAAAAA?text=No+Image' }}" class="article-image" alt="{{ art.title|truncate(50) }}"></a>{% if art.is_stale_bookmark %}<span class="badge bg-secondary position-absolute top-0 end-0 m-2">Cached Bookmark</span>{% endif %}</div>
                        <div class="article-body d-flex flex-column"><h5 class="article-title mb-2"><a href="{{ art.article_url }}" class="text-decoration-none">{{ art.title|truncate(70) }}</a></h5><div class="article-meta small mb-2"><span class="meta-item text-muted"><i class="fas fa-{{ 'user-edit' if art.is_community_article else 'building' }}"></i> {{ art.source.name|truncate(20) }}</span><span class="meta-item text-muted"><i class="far fa-calendar-alt"></i> {{ (art.publishedAt | to_ist if art.publishedAt else 'N/A') }}</span></div><p class="article-description small">{{ art.description|truncate(100) }}</p><a href="{{ art.article_url }}" class="read-more btn btn-sm mt-auto">Read More <i class="fas fa-chevron-right ms-1 small"></i></a></div>
                    </article>
                </div>
                {% endfor %}
            </div>
            {% else %}
                <div class="empty-state-card"><div class="icon"><i class="fas fa-bookmark"></i></div><h4>No Bookmarks Yet</h4><p class="text-muted">You haven't bookmarked any articles. Find an article you like and click the bookmark icon to save it!</p></div>
            {% endif %}
            {% if bookmarks_pagination and bookmarks_pagination.pages > 1 %}<nav aria-label="Bookmarks navigation" class="mt-5"><ul class="pagination justify-content-center"><li class="page-item page-link-prev-next {% if not bookmarks_pagination.has_prev %}disabled{% endif %}"><a class="page-link" href="{{ url_for('profile', page=bookmarks_pagination.prev_num) if bookmarks_pagination.has_prev else '#' }}">&laquo; Prev</a></li>{% for p in bookmarks_pagination.iter_pages(left_edge=1, right_edge=1, left_current=1, right_current=2) %}{% if p %}{% if p == bookmarks_pagination.page %}<li class="page-item active" aria-current="page"><span class="page-link">{{ p }}</span></li>{% else %}<li class="page-item"><a class="page-link" href="{{ url_for('profile', page=p) }}">{{ p }}</a></li>{% endif %}{% else %}<li class="page-item disabled"><span class="page-link">...</span></li>{% endif %}{% endfor %}<li class="page-item page-link-prev-next {% if not bookmarks_pagination.has_next %}disabled{% endif %}"><a class="page-link" href="{{ url_for('profile', page=bookmarks_pagination.next_num) if bookmarks_pagination.has_next else '#' }}">Next &raquo;</a></li></ul></nav>{% endif %}
        </div>
        <div class="tab-pane fade" id="posted-content" role="tabpanel" aria-labelledby="posted-tab">
            {% if posted_articles %}<div class="row g-4">{% for art in posted_articles %}<div class="col-md-6 col-lg-4 d-flex"><article class="article-card d-flex flex-column w-100"><div class="article-image-container"><a href="{{ url_for('article_detail', article_hash_id=art.article_hash_id) }}"><img src="{{ art.image_url or 'https://via.placeholder.com/400x220/EEEEEE/AAAAAA?text=No+Image' }}" class="article-image" alt="{{ art.title|truncate(50) }}"></a></div><div class="article-body d-flex flex-column"><h5 class="article-title mb-2"><a href="{{ url_for('article_detail', article_hash_id=art.article_hash_id) }}" class="text-decoration-none">{{ art.title|truncate(70) }}</a></h5><div class="article-meta small mb-2"><span class="meta-item text-muted"><i class="fas fa-user-edit"></i> {{ art.author.name|truncate(20) }}</span><span class="meta-item text-muted"><i class="far fa-calendar-alt"></i> {{ art.published_at | to_ist }}</span></div><p class="article-description small">{{ art.description|truncate(100) }}</p><a href="{{ url_for('article_detail', article_hash_id=art.article_hash_id) }}" class="read-more btn btn-sm mt-auto">Read More <i class="fas fa-chevron-right ms-1 small"></i></a></div></article></div>{% endfor %}</div>{% else %}<div class="empty-state-card"><div class="icon"><i class="fas fa-feather-alt"></i></div><h4>Nothing Posted Yet</h4><p class="text-muted">You haven't posted any articles. Click the <i class="fas fa-pen-to-square"></i> button to share your first story!</p></div>{% endif %}
        </div>
    </div>
</div>
{% endblock %}
...
<script>
document.addEventListener('DOMContentLoaded', function () {
    // Logic for activating the correct tab if linked via hash
    ...
    const adminPanel = document.getElementById('admin-panel-tab-pane');
    if (adminPanel) {
        adminPanel.addEventListener('click', function(event) {
            const dismissBtn = event.target.closest('.dismiss-report-btn');
            const deleteBtn = event.target.closest('.delete-article-btn');

            if (dismissBtn) {
                handleAdminAction(dismissBtn, `/admin/dismiss_report/${dismissBtn.dataset.reportId}`, 'Dismiss this report?', 'Dismissed');
            }
            if (deleteBtn) {
                handleAdminAction(deleteBtn, `/admin/delete_article/${deleteBtn.dataset.articleHashId}`, 'Permanently delete this article? This cannot be undone.', 'Deleted');
            }
        });
    }

    function handleAdminAction(button, url, confirmMessage, successVerb) {
        ...
        .then(({ ok, data }) => {
            if (ok) {
                console.log(`Successfully ${successVerb.toLowerCase()}.`);
                row.style.transition = 'opacity 0.5s ease';
                row.style.opacity = '0';
                setTimeout(() => row.remove(), 500);
                // Optionally, create a flash message dynamically if needed
            } else {
...
# The remaining templates are included for completeness as requested.
# Their content is identical to the provided original code.
# --- Start of Unchanged Templates ---
INDEX_HTML_TEMPLATE = """
{% extends "BASE_HTML_TEMPLATE" %}
{% block title %}
...
{% if is_main_homepage %}
    <div class="animate-fade-in">
        ...
        <div class="ai-synthesis-card">
            <div class="synthesis-header"><i class="fas fa-brain"></i><h2>Today's Briefing: The Big Picture</h2></div>
            <p class="synthesis-text">"{{ synthesis }}"</p>
            {% if keywords %}
            <div class="synthesis-keywords">
                {% for keyword in keywords %}<a href="{{ url_for('search_results', query=keyword) }}" class="keyword-tag">{{ keyword }}</a>{% endfor %}
            </div>
            {% endif %}
        </div>
        {% endif %}
        {% if featured_article %}
        <article class="featured-story row g-0">
            <div class="col-md-7 featured-story-image" style="background-image: url('{{ featured_article.urlToImage }}')"></div>
            <div class="col-md-5 featured-story-content">
                <div class="article-meta"><span class="meta-item"><i class="fas fa-fire-alt text-danger"></i> Top Story</span><span class="meta-item"><i class="fas fa-building"></i> {{ featured_article.source.name|truncate(20) }}</span></div>
        ...
        {% endif %}
        <ul class="nav nav-tabs nav-fill mb-3" id="newsTab" role="tablist" style="font-weight: 600;"><li class="nav-item" role="presentation"><button class="nav-link active" id="popular-tab" data-bs-toggle="tab" data-bs-target="#popular-tab-pane" type="button" role="tab" aria-controls="popular-tab-pane" aria-selected="true"><i class="fas fa-fire-alt me-1"></i> POPULAR STORIES</button></li><li class="nav-item" role="presentation"><button class="nav-link" id="yesterday-tab" data-bs-toggle="tab" data-bs-target="#yesterday-tab-pane" type="button" role="tab" aria-controls="yesterday-tab-pane" aria-selected="false"><i class="fas fa-history me-1"></i> YESTERDAY'S HEADLINES</button></li></ul>
        <div class="tab-content" id="newsTabContent">
            <div class="tab-pane fade show active" id="popular-tab-pane" role="tabpanel" aria-labelledby="popular-tab"><div class="row g-4 pt-3">{% if popular_articles %}{% for art in popular_articles %}<div class="col-md-6 col-lg-4 d-flex"><article class="article-card d-flex flex-column w-100">{% set article_url = url_for('article_detail', article_hash_id=art.id) %}<div class="article-image-container"><a href="{{ article_url }}"><img src="{{ art.urlToImage }}" class="article-image" alt="{{ art.title|truncate(50) }}"></a></div><div class="article-body d-flex flex-column"><div class="d-flex justify-content-between align-items-start"><h5 class="article-title mb-2 flex-grow-1"><a href="{{ article_url }}" class="text-decoration-none">{{ art.title|truncate(70) }}</a></h5>{% if session.user_id %}<button class="bookmark-btn homepage-bookmark-btn {% if art.is_bookmarked %}active{% endif %}" style="margin-left: 10px; padding-top:0;" title="Bookmark" data-article-hash-id="{{ art.id }}" data-is-community="false" data-title="{{ art.title|e }}" data-source-name="{{ art.source.name|e }}" data-image-url="{{ art.urlToImage|e }}" data-description="{{ (art.description if art.description else '')|e }}" data-published-at="{{ (art.publishedAt if art.publishedAt else '')|e }}"><i class="fa-solid fa-bookmark"></i></button>{% endif %}</div><div class="article-meta small mb-2"><span class="meta-item text-muted"><i class="fas fa-building"></i> {{ art.source.name|truncate(20) }}</span><span class="meta-item text-muted"><i class="far fa-calendar-alt"></i> {{ (art.publishedAt | to_ist if art.publishedAt else 'N/A') }}</span></div><p class="article-description small">{{ art.description|truncate(100) }}</p><a href="{{ article_url }}" class="read-more btn btn-sm mt-auto">Read More <i class="fas fa-chevron-right ms-1 small"></i></a></div></article></div>{% endfor %}{% else %}<div class="col-12"><div class="alert alert-light text-center">More popular stories are currently unavailable.</div></div>{% endif %}</div>{% if popular_articles %}<div class="text-center mt-4"><a href="{{ url_for('index', category_name='Popular Stories') }}" class="btn btn-outline-primary">View All Popular Stories <i class="fas fa-arrow-right ms-1"></i></a></div>{% endif %}</div>
            <div class="tab-pane fade" id="yesterday-tab-pane" role="tabpanel" aria-labelledby="yesterday-tab"><div class="row g-4 pt-3">{% if latest_yesterday_articles %}{% for art in latest_yesterday_articles %}<div class="col-md-6 col-lg-4 d-flex"><article class="article-card d-flex flex-column w-100">{% set article_url = url_for('article_detail', article_hash_id=art.id) %}<div class="article-image-container"><a href="{{ article_url }}"><img src="{{ art.urlToImage }}" class="article-image" alt="{{ art.title|truncate(50) }}"></a></div><div class="article-body d-flex flex-column"><div class="d-flex justify-content-between align-items-start"><h5 class="article-title mb-2 flex-grow-1"><a href="{{ article_url }}" class="text-decoration-none">{{ art.title|truncate(70) }}</a></h5>{% if session.user_id %}<button class="bookmark-btn homepage-bookmark-btn {% if art.is_bookmarked %}active{% endif %}" style="margin-left: 10px; padding-top:0;" title="Bookmark" data-article-hash-id="{{ art.id }}" data-is-community="false" data-title="{{ art.title|e }}" data-source-name="{{ art.source.name|e }}" data-image-url="{{ art.urlToImage|e }}" data-description="{{ (art.description if art.description else '')|e }}" data-published-at="{{ (art.publishedAt if art.publishedAt else '')|e }}"><i class="fa-solid fa-bookmark"></i></button>{% endif %}</div><div class="article-meta small mb-2"><span class="meta-item text-muted"><i class="fas fa-building"></i> {{ art.source.name|truncate(20) }}</span><span class="meta-item text-muted"><i class="far fa-calendar-alt"></i> {{ (art.publishedAt | to_ist if art.publishedAt else 'N/A') }}</span></div><p class="article-description small">{{ art.description|truncate(100) }}</p><a href="{{ article_url }}" class="read-more btn btn-sm mt-auto">Read More <i class="fas fa-chevron-right ms-1 small"></i></a></div></article></div>{% endfor %}{% else %}<div class="col-12"><div class="alert alert-light text-center">Could not load yesterday's articles.</div></div>{% endif %}</div>{% if latest_yesterday_articles %}<div class="text-center mt-4"><a href="{{ url_for('index', category_name="Yesterday's Headlines") }}" class="btn btn-outline-primary">View All of Yesterday's Headlines <i class="fas fa-arrow-right ms-1"></i></a></div>{% endif %}</div>
        </div>
    </div>
{% else %}
    {% if selected_category == 'All Articles' and current_filter_date %}<h4 class="mb-3 fst-italic">Showing articles for: {{ current_filter_date }}</h4>{% elif selected_category != 'All Articles' and selected_category != 'Community Hub' %}<h2 class="pb-2 border-bottom mb-4">{{ selected_category }}</h2>{% endif %}
    {% if articles and not is_main_homepage %}<div class="row g-4">{% for art in articles %}<div class="col-md-6 col-lg-4 d-flex"><article class="article-card animate-fade-in d-flex flex-column w-100" style="animation-delay: {{ loop.index0 * 0.05 }}s">{% set article_url = url_for('article_detail', article_hash_id=(art.article_hash_id if art.is_community_article else art.id)) %}<div class="article-image-container"><a href="{{ article_url }}"><img src="{{ art.image_url if art.is_community_article else art.urlToImage }}" class="article-image" alt="{{ art.title|truncate(50) }}"></a></div><div class="article-body d-flex flex-column"><div class="d-flex justify-content-between align-items-start"><h5 class="article-title mb-2 flex-grow-1"><a href="{{ article_url }}" class="text-decoration-none">{{ art.title|truncate(70) }}</a></h5>{% if session.user_id %}<button class="bookmark-btn homepage-bookmark-btn {% if art.is_bookmarked %}active{% endif %}" style="margin-left: 10px; padding-top:0;" title="{% if art.is_bookmarked %}Remove Bookmark{% else %}Add Bookmark{% endif %}" data-article-hash-id="{{ art.article_hash_id if art.is_community_article else art.id }}" data-is-community="{{ 'true' if art.is_community_article else 'false' }}" data-title="{{ art.title|e }}" data-source-name="{{ (art.author.name if art.is_community_article and art.author else art.source.name)|e }}" data-image-url="{{ (art.image_url if art.is_community_article else art.urlToImage)|e }}" data-description="{{ (art.description if art.description else '')|e }}" data-published-at="{{ (art.published_at.isoformat() if art.is_community_article and art.published_at else (art.publishedAt if not art.is_community_article and art.publishedAt else ''))|e }}"><i class="fa-solid fa-bookmark"></i></button>{% endif %}</div><div class="article-meta small mb-2"><span class="meta-item text-muted"><i class="fas fa-{{ 'user-edit' if art.is_community_article else 'building' }}"></i> {% if art.is_community_article and art.author %}<a href="{{ url_for('public_profile', username=art.author.username) }}" class="text-muted text-decoration-none">{{ art.author.name|truncate(20) }}{% if art.author.is_admin %}<span class="admin-badge ms-1">Admin</span>{% endif %}</a>{% else %}{{ art.source.name|truncate(20) }}{% endif %}</span><span class="meta-item text-muted"><i class="far fa-calendar-alt"></i> {{ (art.published_at | to_ist if art.is_community_article else (art.publishedAt | to_ist if art.publishedAt else 'N/A')) }}</span></div><p class="article-description small">{{ art.description|truncate(100) }}</p><a href="{{ article_url }}" class="read-more btn btn-sm mt-auto">Read More <i class="fas fa-chevron-right ms-1 small"></i></a></div></article></div>{% endfor %}</div>
    {% elif not articles %}<div class="alert alert-info text-center my-5 p-4"><h4><i class="fas fa-search me-2"></i>No articles found.</h4><p>Please try a different category or search query.</p></div>{% endif %}
    {% if total_pages and total_pages > 1 %}<nav aria-label="Page navigation" class="mt-5"><ul class="pagination justify-content-center">{% set filter_date_for_url = request.args.get('filter_date') if selected_category == 'All Articles' and request.args.get('filter_date') else None %}<li class="page-item page-link-prev-next {% if current_page == 1 %}disabled{% endif %}"><a class="page-link" href="{{ url_for(request.endpoint, page=current_page-1, category_name=selected_category if request.endpoint != 'search_results' else None, query=query if request.endpoint == 'search_results' else None, filter_date=filter_date_for_url) if current_page > 1 else '#' }}">&laquo; Prev</a></li>{% set page_window = 1 %}{% set show_first = 1 %}{% set show_last = total_pages %}{% if current_page - page_window > show_first %}<li class="page-item"><a class="page-link" href="{{ url_for(request.endpoint, page=1, category_name=selected_category if request.endpoint != 'search_results' else None, query=query if request.endpoint == 'search_results' else None, filter_date=filter_date_for_url) }}">1</a></li>{% if current_page - page_window > show_first + 1 %}<li class="page-item disabled"><span class="page-link">...</span></li>{% endif %}{% endif %}{% for p in range(1, total_pages + 1) %}{% if p == current_page %}<li class="page-item active" aria-current="page"><span class="page-link">{{ p }}</span></li>{% elif p >= current_page - page_window and p <= current_page + page_window %}<li class="page-item"><a class="page-link" href="{{ url_for(request.endpoint, page=p, category_name=selected_category if request.endpoint != 'search_results' else None, query=query if request.endpoint == 'search_results' else None, filter_date=filter_date_for_url) }}">{{ p }}</a></li>{% endif %}{% endfor %}{% if current_page + page_window < show_last %}{% if current_page + page_window < show_last - 1 %}<li class="page-item disabled"><span class="page-link">...</span></li>{% endif %}<li class="page-item"><a class="page-link" href="{{ url_for(request.endpoint, page=total_pages, category_name=selected_category if request.endpoint != 'search_results' else None, query=query if request.endpoint == 'search_results' else None, filter_date=filter_date_for_url) }}">{{ total_pages }}</a></li>{% endif %}<li class="page-item page-link-prev-next {% if current_page == total_pages %}disabled{% endif %}"><a class="page-link" href="{{ url_for(request.endpoint, page=current_page+1, category_name=selected_category if request.endpoint != 'search_results' else None, query=query if request.endpoint == 'search_results' else None, filter_date=filter_date_for_url) if current_page < total_pages else '#' }}">Next &raquo;</a></li></ul></nav>{% endif %}
{% endif %}
{% endblock %}
{% block scripts_extra %}
<script>
...
{% block head_extra %}<style>.article-full-content-wrapper { background-color: var(--card-bg); padding: clamp(1rem, 4vw, 2rem); border-radius: var(--border-radius-lg); box-shadow: var(--shadow-md); margin-bottom: 2rem; margin-top: 1rem; } .article-title-main {font-weight: 700; color: var(--text-color); line-height:1.3; font-family: 'Poppins', sans-serif;} .summary-box, .takeaways-box { background-color: rgba(var(--primary-color-rgb), 0.04); border: 1px solid rgba(var(--primary-color-rgb), 0.1); border-radius: var(--border-radius-md); margin: 1.5rem 0; padding: 1.5rem; } body.dark-mode .summary-box, body.dark-mode .takeaways-box { background-color: #2a30422e; } .takeaways-box { border-left: 4px solid var(--secondary-color); } .loader-container { display: flex; flex-direction: column; justify-content: center; align-items: center; min-height: 200px; padding: 2rem; color: var(--text-muted-color); } .loader { border: 5px solid var(--light-bg); border-top: 5px solid var(--primary-color); border-radius: 50%; width: 50px; height: 50px; animation: spin 1s linear infinite; margin-bottom: 1rem; } .content-text { white-space: pre-wrap; line-height: 1.8; font-size: 1.05rem; } @keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } } .edit-form-container { display: none; }</style>{% endblock %}
{% block content %}{% if not article %}<div class="alert alert-danger text-center my-5 p-4"><h4><i class="fas fa-exclamation-triangle me-2"></i>Article Not Found</h4><p>The article you are looking for could not be found.</p><a href="{{ url_for('index') }}" class="btn btn-primary mt-2">Go to Homepage</a></div>{% else %}<article class="article-full-content-wrapper animate-fade-in"><div class="mb-3 d-flex justify-content-between align-items-center"><a href="{{ previous_list_page }}" class="btn btn-sm btn-outline-secondary"><i class="fas fa-arrow-left me-2"></i>Back to List</a><div class="d-flex align-items-center gap-2">{% if session.user_id %}{% if is_community_article %}<button id="reportBtn" class="btn btn-sm btn-outline-warning" title="Report this article for review"><i class="fas fa-flag"></i> Report</button>{% endif %}<button id="bookmarkBtn" class="bookmark-btn {% if is_bookmarked %}active{% endif %}" title="{% if is_bookmarked %}Remove Bookmark{% else %}Add Bookmark{% endif %}" data-article-hash-id="{{ article.article_hash_id if is_community_article else article.id }}" data-is-community="{{ 'true' if is_community_article else 'false' }}" data-title="{{ article.title|e }}" data-source-name="{{ (article.author.name if is_community_article and article.author else article.source.name)|e }}" data-image-url="{{ (article.image_url if is_community_article else article.urlToImage)|e }}" data-description="{{ (article.description if article.description else '')|e }}" data-published-at="{{ (article.published_at.isoformat() if is_community_article and article.published_at else (article.publishedAt if not is_community_article and article.publishedAt else ''))|e }}"><i class="fa-solid fa-bookmark"></i></button>{% endif %}</div></div><h1 class="mb-2 article-title-main display-6">{{ article.title }}</h1><div class="article-meta-detailed d-flex align-items-center flex-wrap gap-3 text-muted small"><span class="meta-item" title="Source"><i class="fas fa-{{ 'user-edit' if is_community_article else 'building' }}"></i> {{ article.author.name if is_community_article and article.author else article.source.name }}</span><span class="meta-item" title="Published Date"><i class="far fa-calendar-alt"></i> {{ (article.published_at | to_ist if is_community_article else (article.publishedAt | to_ist if article.publishedAt else 'N/A')) }}</span></div>{% set image_to_display = article.image_url if is_community_article else article.urlToImage %}{% if image_to_display %}<img src="{{ image_to_display }}" alt="{{ article.title|truncate(50) }}" class="img-fluid rounded my-3 shadow-sm">{% endif %}<div id="contentLoader" class="loader-container my-4 {% if is_community_article %}d-none{% endif %}"><div class="loader"></div><div>Analyzing article and generating summary...</div></div><div id="articleAnalysisContainer">{% if is_community_article %}{% if article.groq_summary %}<div class="summary-box my-3"><h5><i class="fas fa-book-open me-2"></i>AI Summary</h5><p class="mb-0">{{ article.groq_summary|replace('\\n', '<br>')|safe }}</p></div>{% endif %}{% if article.parsed_takeaways %}<div class="takeaways-box my-3"><h5><i class="fas fa-list-check me-2"></i>AI Key Takeaways</h5><ul>{% for takeaway in article.parsed_takeaways %}<li>{{ takeaway }}</li>{% endfor %}</ul></div>{% endif %}<hr class="my-4"><h4 class="mb-3">Full Article Content</h4><div class="content-text">{{ article.full_text }}</div>{% else %}<div id="apiArticleContent"></div>{% endif %}</div><section class="comment-section mt-5" id="comment-section"><h3 class="mb-4">Community Discussion (<span id="comment-count">{{ total_comment_count }}</span>)</h3><div id="comments-list">{% for comment in comments %}{% include '_COMMENT_TEMPLATE' %}{% else %}<p id="no-comments-msg" class="text-muted mt-3">No comments yet. Be the first to share your thoughts!</p>{% endfor %}</div>{% if session.user_id %}<div class="add-comment-form mt-4 pt-4 border-top"><h5 class="mb-3">Leave a Comment</h5><form id="comment-form"><div class="mb-3"><textarea class="form-control" id="comment-content" name="content" rows="4" placeholder="Share your insights..." required></textarea></div><button type="submit" class="btn btn-primary">Post Comment</button></form></div>{% else %}<div class="alert alert-light mt-4 text-center">Please <a href="{{ url_for('login', next=request.url) }}" class="fw-bold">log in</a> to join the discussion.</div>{% endif %}</section></article>{% endif %}{% endblock %}
{% block scripts_extra %}<script>document.addEventListener('DOMContentLoaded', function () { try { {% if article %} const articleHashIdGlobal = {{ (article.article_hash_id if is_community_article else article.id) | tojson }}; const isUserLoggedIn = {{ 'true' if session.user_id else 'false' }}; const isCommunityArticle = {{ is_community_article | tojson }}; if (!isCommunityArticle) { const contentLoader = document.getElementById('contentLoader'); const apiArticleContent = document.getElementById('apiArticleContent'); fetch(`{{ url_for('get_article_content_json', article_hash_id='PLACEHOLDER') }}`.replace('PLACEHOLDER', articleHashIdGlobal)).then(response => { if (!response.ok) { throw new Error(`Network error, status: ${response.status}`); } return response.json(); }).then(data => { if (data.error) { throw new Error(data.error); } let html = ''; const articleUrl = {{ article.url | tojson if article and not is_community_article else 'null' }}; const articleSourceName = {{ article.source.name | tojson if article and not is_community_article and article.source else 'Source'|tojson }}; const analysis = data.groq_analysis; if (analysis) { if (analysis.error) { html += `<div class="alert alert-secondary small p-3 mt-3">AI analysis could not be performed: ${analysis.error}</div>`; } else { if (analysis.groq_summary) { html += `<div class="summary-box my-3"><h5><i class="fas fa-book-open me-2"></i>AI Summary</h5><p class="mb-0">${analysis.groq_summary.replace(/\\n/g, '<br>')}</p></div>`; } if (analysis.groq_takeaways && analysis.groq_takeaways.length > 0) { html += `<div class="takeaways-box my-3"><h5><i class="fas fa-list-check me-2"></i>AI Key Takeaways</h5><ul>${analysis.groq_takeaways.map(t => `<li>${String(t)}</li>`).join('')}</ul></div>`; } } } if (articleUrl) { html += `<hr class="my-4"><a href="${articleUrl}" class="btn btn-outline-primary mt-3 mb-3" target="_blank" rel="noopener noreferrer">Read Original Article at ${articleSourceName} <i class="fas fa-external-link-alt ms-1"></i></a>`; } apiArticleContent.innerHTML = html; }).catch(error => { console.error("Failed to load article content:", error); if (apiArticleContent) { apiArticleContent.innerHTML = `<div class="alert alert-danger small p-3">Failed to load article analysis. Details: ${error.message}</div>`; } }).finally(() => { if (contentLoader) contentLoader.style.display = 'none'; }); } const commentSection = document.getElementById('comment-section'); if (commentSection && isUserLoggedIn) { const handleCommentSubmit = (formElement) => { const content = formElement.querySelector('textarea[name="content"]').value; const parentId = formElement.querySelector('input[name="parent_id"]')?.value || null; if (!content.trim()) return; const submitButton = formElement.querySelector('button[type="submit"]'); const originalButtonText = submitButton.innerHTML; submitButton.disabled = true; submitButton.innerHTML = '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Posting...'; fetch(`{{ url_for('add_comment', article_hash_id='PLACEHOLDER') }}`.replace('PLACEHOLDER', articleHashIdGlobal), { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ content, parent_id: parentId }) }).then(res => { if (res.status === 401) { throw new Error("Your session has expired. Please refresh the page and log in again."); } if (!res.ok) { return res.json().then(err => { throw new Error(err.error || "An unknown server error occurred."); }); } return res.json(); }).then(data => { if (data.success) { const noCommentsMsg = document.getElementById('no-comments-msg'); if (noCommentsMsg) noCommentsMsg.remove(); if (data.parent_id) { document.getElementById(`replies-of-${data.parent_id}`).insertAdjacentHTML('beforeend', data.html); formElement.closest('.reply-form-container').style.display = 'none'; } else { document.getElementById('comments-list').insertAdjacentHTML('beforeend', data.html); } const countEl = document.getElementById('comment-count'); countEl.textContent = parseInt(countEl.textContent) + 1; formElement.reset(); } else { throw new Error(data.error || 'Could not post comment.'); } }).catch(err => { console.error("Comment submission error:", err); alert("Error: " + err.message); }).finally(() => { submitButton.disabled = false; submitButton.innerHTML = originalButtonText; }); }; const updateReactionUI = (commentId, reactions, userReaction) => { const summaryContainer = document.getElementById(`reaction-summary-${commentId}`); if (!summaryContainer) return; let summaryHTML = ''; if (reactions) { for (const [emoji, count] of Object.entries(reactions)) { if (count > 0) { const userReactedClass = (userReaction === emoji) ? 'user-reacted' : ''; summaryHTML += `<div class="reaction-pill ${userReactedClass}" data-emoji="${emoji}"><span class="emoji">${emoji}</span> <span class="count">${count}</span></div>`; } } } summaryContainer.innerHTML = summaryHTML; }; commentSection.addEventListener('click', function(e) { const target = e.target; const deleteBtn = target.closest('.delete-btn'); if (deleteBtn) { e.preventDefault(); const commentId = deleteBtn.dataset.commentId; if (confirm('Are you sure you want to delete this comment? All replies will also be removed.')) { fetch(`/delete_comment/${commentId}`, { method: 'POST' }).then(res => { if (!res.ok) { return res.json().then(err => { throw new Error(err.error) }); } return res.json(); }).then(data => { if (data.success) { const commentElement = document.getElementById(`comment-${commentId}`); const repliesCount = commentElement.querySelectorAll('.comment-thread').length; const totalCommentsToRemove = 1 + repliesCount; const countEl = document.getElementById('comment-count'); countEl.textContent = Math.max(0, parseInt(countEl.textContent) - totalCommentsToRemove); commentElement.style.transition = 'opacity 0.5s ease'; commentElement.style.opacity = '0'; setTimeout(() => commentElement.remove(), 500); } else { alert('Error: ' + data.error); } }).catch(err => { console.error("Delete error:", err); alert("Could not delete comment: " + err.message); }); } return; } const editBtn = target.closest('.edit-btn'); if (editBtn) { e.preventDefault(); const commentId = editBtn.dataset.commentId; const commentThread = document.getElementById(`comment-${commentId}`); const commentBody = commentThread.querySelector('.comment-body'); commentBody.querySelector('.comment-content').style.display = 'none'; commentBody.querySelector('.comment-actions').style.display = 'none'; commentBody.querySelector('.edit-form-container').style.display = 'block'; return; } const cancelEditBtn = target.closest('.cancel-edit-btn'); if (cancelEditBtn) { e.preventDefault(); const commentBody = cancelEditBtn.closest('.comment-body'); commentBody.querySelector('.comment-content').style.display = 'block'; commentBody.querySelector('.comment-actions').style.display = 'flex'; commentBody.querySelector('.edit-form-container').style.display = 'none'; return; } const replyBtn = target.closest('.reply-btn'); if (replyBtn) { e.preventDefault(); const commentId = replyBtn.dataset.commentId; const formContainer = document.getElementById(`reply-form-container-${commentId}`); if (formContainer) { const isDisplayed = formContainer.style.display === 'block'; document.querySelectorAll('.reply-form-container').forEach(fc => fc.style.display = 'none'); formContainer.style.display = isDisplayed ? 'none' : 'block'; if (!isDisplayed) formContainer.querySelector('textarea').focus(); } return; } const reactBtn = target.closest('.react-btn'); if (reactBtn) { e.preventDefault(); const commentId = reactBtn.dataset.commentId; const reactionBox = document.getElementById(`reaction-box-${commentId}`); if (reactionBox) { const isShown = reactionBox.classList.contains('show'); document.querySelectorAll('.reaction-box').forEach(box => box.classList.remove('show')); if (!isShown) reactionBox.classList.add('show'); } return; } const reactionEmoji = target.closest('.reaction-emoji'); if (reactionEmoji) { e.preventDefault(); const commentId = reactionEmoji.dataset.commentId; const emoji = reactionEmoji.dataset.emoji; reactionEmoji.closest('.reaction-box').classList.remove('show'); fetch(`/vote_comment/${commentId}`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' }, body: JSON.stringify({ emoji: emoji }) }).then(res => res.json()).then(data => { if (data.success) { updateReactionUI(commentId, data.reactions, data.user_reaction); } else { throw new Error(data.error || "Failed to vote."); } }).catch(err => { console.error("Reaction error:", err); alert("Error: " + err.message); }); return; } if (!target.closest('.reaction-box') && !target.closest('.react-btn')) { document.querySelectorAll('.reaction-box.show').forEach(box => box.classList.remove('show')); } }); commentSection.addEventListener('submit', function(e) { e.preventDefault(); if (e.target.matches('.edit-comment-form')) { const form = e.target; const commentId = form.closest('.comment-thread').id.replace('comment-', ''); const newContent = form.querySelector('textarea[name="content"]').value.trim(); if (!newContent) return; fetch(`/edit_comment/${commentId}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: newContent }) }).then(res => { if (!res.ok) { return res.json().then(err => { throw new Error(err.error) }); } return res.json(); }).then(data => { if (data.success) { const commentBody = form.closest('.comment-body'); const contentP = commentBody.querySelector('.comment-content'); contentP.textContent = data.new_content; contentP.style.display = 'block'; commentBody.querySelector('.comment-actions').style.display = 'flex'; form.closest('.edit-form-container').style.display = 'none'; } else { alert('Error: ' + data.error); } }).catch(err => { console.error("Edit error:", err); alert("Could not save changes: " + err.message); }); return; } if (e.target.id === 'comment-form' || e.target.matches('.reply-form')) { handleCommentSubmit(e.target); } }); } const reportBtn = document.getElementById('reportBtn'); if (reportBtn) { reportBtn.addEventListener('click', function() { if (!confirm('Are you sure you want to report this article for review?')) { return; } this.disabled = true; this.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Reporting...'; fetch(`/report_article/${articleHashIdGlobal}`, { method: 'POST', headers: { 'Content-Type': 'application/json' } }).then(res => res.json().then(data => ({ ok: res.ok, status: res.status, data }))).then(({ ok, status, data }) => { if (ok) { this.innerHTML = '<i class="fas fa-check"></i> Reported'; alert(data.message); } else { this.disabled = false; this.innerHTML = '<i class="fas fa-flag"></i> Report'; alert('Error: ' + data.error); } }).catch(err => { console.error("Report error:", err); alert("A network error occurred. Please try again."); this.disabled = false; this.innerHTML = '<i class="fas fa-flag"></i> Report'; }); }); } const bookmarkBtn = document.getElementById('bookmarkBtn'); if (bookmarkBtn && isUserLoggedIn) { bookmarkBtn.addEventListener('click', function() { const articleHashId = this.dataset.articleHashId; const isCommunity = this.dataset.isCommunity; const title = this.dataset.title; const sourceName = this.dataset.sourceName; const imageUrl = this.dataset.imageUrl; const description = this.dataset.description; const publishedAt = this.dataset.publishedAt; fetch(`{{ url_for('toggle_bookmark', article_hash_id='PLACEHOLDER') }}`.replace('PLACEHOLDER', articleHashId), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ is_community_article: isCommunity, title, source_name: sourceName, image_url: imageUrl, description, published_at: publishedAt }) }).then(res => res.json()).then(data => { if (data.success) { this.classList.toggle('active', data.status === 'added'); this.title = data.status === 'added' ? 'Remove Bookmark' : 'Add Bookmark'; } else { alert('Error: ' + (data.error || 'Could not update bookmark.')); } }).catch(err => { console.error("Bookmark error:", err); alert("Could not update bookmark: " + err.message); }); }); } {% endif %} } catch (e) { console.error("A critical error occurred on the article page:", e); } });</script>{% endblock %}
"""
LOGIN_HTML_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}Login - BrieflyAI{% endblock %}{% block body_class %}body-auth{% endblock %}{% block content %}<div class="auth-card animate-fade-in"><div class="auth-header"><div class="brand-icon"><i class="fas fa-bolt-lightning"></i></div><h2>Welcome Back to BrieflyAI</h2></div><div class="auth-body"><form method="POST" action="{{ url_for('login', next=request.args.get('next')) }}"><div class="mb-3"><label for="username" class="form-label fw-medium">Username</label><div class="input-group-icon"><i class="fas fa-user input-icon"></i><input type="text" class="form-control" id="username" name="username" required placeholder="e.g. user123"></div></div><div class="mb-4"><label for="password" class="form-label fw-medium">Password</label><div class="input-group-icon"><i class="fas fa-lock input-icon"></i><input type="password" class="form-control" id="password" name="password" required placeholder="••••••••"></div></div><button type="submit" class="btn btn-primary w-100">Sign In</button></form></div><div class="auth-footer"><p class="mb-0 small">Don't have an account? <a href="{{ url_for('register', next=request.args.get('next')) }}" class="fw-bold text-decoration-none">Sign up now</a></p></div></div>{% endblock %}"""
REGISTER_HTML_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}Register - BrieflyAI{% endblock %}{% block body_class %}body-auth{% endblock %}{% block content %}<div class="auth-card animate-fade-in"><div class="auth-header"><div class="brand-icon"><i class="fas fa-user-plus"></i></div><h2>Create Your Account</h2></div><div class="auth-body"><form method="POST" action="{{ url_for('register') }}"><div class="mb-3"><label for="name" class="form-label fw-medium">Full Name</label><div class="input-group-icon"><i class="fas fa-id-card input-icon"></i><input type="text" class="form-control" id="name" name="name" required placeholder="e.g. John Doe"></div></div><div class="mb-3"><label for="username" class="form-label fw-medium">Username</label><div class="input-group-icon"><i class="fas fa-user input-icon"></i><input type="text" class="form-control" id="username" name="username" required placeholder="e.g. johndoe (min 3 chars)"></div></div><div class="mb-4"><label for="password" class="form-label fw-medium">Password</label><div class="input-group-icon"><i class="fas fa-lock input-icon"></i><input type="password" class="form-control" id="password" name="password" required placeholder="min 6 chars"></div></div><button type="submit" class="btn btn-primary w-100">Create Account</button></form></div><div class="auth-footer"><p class="mb-0 small">Already have an account? <a href="{{ url_for('login') }}" class="fw-bold text-decoration-none">Sign In</a></p></div></div>{% endblock %}"""
PUBLIC_PROFILE_HTML_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}{{ user.name }}'s Profile - BrieflyAI{% endblock %}{% block content %}<div class="animate-fade-in"><div class="profile-header-card mb-4"><div class="profile-avatar-wrapper"><div class="profile-avatar">{{ user.name[0]|upper }}</div></div><h2>{{ user.name }} {% if user.is_admin %}<span class="admin-badge">Admin</span>{% endif %}</h2><p class="username">@{{ user.username }}</p><p class="small text-muted mb-0">Member Since: {{ user.created_at | to_ist }}</p></div><h3 class="mt-5 mb-4">Articles by {{ user.name }} ({{ posted_articles|length }})</h3><div class="row g-4">{% if posted_articles %}{% for art in posted_articles %}<div class="col-md-6 col-lg-4 d-flex"><article class="article-card d-flex flex-column w-100">{% set article_url = url_for('article_detail', article_hash_id=art.article_hash_id) %}<div class="article-image-container"><a href="{{ article_url }}"><img src="{{ art.image_url if art.image_url else 'https://via.placeholder.com/400x220/EEEEEE/AAAAAA?text=No+Image' }}" class="article-image" alt="{{ art.title|truncate(50) }}"></a></div><div class="article-body d-flex flex-column"><h5 class="article-title mb-2"><a href="{{ article_url }}" class="text-decoration-none">{{ art.title|truncate(70) }}</a></h5><div class="article-meta small mb-2"><span class="meta-item text-muted"><i class="far fa-calendar-alt"></i> {{ art.published_at | to_ist }}</span></div><p class="article-description small">{{ art.description|truncate(100) }}</p><a href="{{ article_url }}" class="read-more btn btn-sm mt-auto">Read More <i class="fas fa-chevron-right ms-1 small"></i></a></div></article></div>{% endfor %}{% else %}<div class="col-12"><div class="empty-state-card"><div class="icon"><i class="fas fa-feather-alt"></i></div><h4>No Articles Yet</h4><p class="text-muted">{{ user.name }} hasn't posted any articles yet.</p></div></div>{% endif %}</div></div>{% endblock %}"""
ABOUT_US_HTML_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}About Us - BrieflyAI{% endblock %}{% block content %}<div class="animate-fade-in"><div class="page-header-static"><h1>About BrieflyAI</h1></div><div class="static-content-container"><p class="lead">Welcome to BrieflyAI, your premier destination for the latest news from India and around the world, delivered in a concise and easy-to-digest format. We leverage the power of cutting-edge AI to summarize complex news articles into key takeaways, saving you time while keeping you informed.</p><h2><i class="icon fas fa-bullseye"></i>Our Mission</h2><p>In a world of information overload, our mission is to provide clarity and efficiency. We believe that everyone deserves access to accurate, unbiased news without spending hours sifting through lengthy articles. BrieflyAI cuts through the noise, offering insightful summaries that matter.</p><h2><i class="icon fas fa-users"></i>Community Hub</h2><p>Beyond AI-driven news, BrieflyAI is a platform for discussion and community engagement. Our Community Hub allows users to post their own articles, share perspectives, and engage in meaningful conversations about the topics that shape our world. We are committed to fostering a respectful and intelligent environment for all our members.</p><h2><i class="icon fas fa-microchip"></i>Our Technology</h2><p>We use state-of-the-art Natural Language Processing (NLP) models to analyze and summarize news content from trusted sources. Our system is designed to identify the most crucial points of an article, presenting them as a quick summary and a list of key takeaways, ensuring you get the essence of the story in seconds.</p></div></div>{% endblock %}"""
CONTACT_HTML_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}Contact Us - BrieflyAI{% endblock %}{% block content %}<div class="animate-fade-in"><div class="page-header-static"><h1>Get In Touch</h1></div><div class="static-content-container"><p class="lead text-center mb-5">We'd love to hear from you! Whether you have a question, feedback, or a news tip, feel free to reach out using one of the methods below.</p><div class="row g-4"><div class="col-md-6"><div class="contact-card"><div class="icon"><i class="fas fa-envelope"></i></div><h4 class="h5">General Inquiries</h4><p class="text-muted">For general questions, feedback, or support, please email us at:</p><a href="mailto:vbansal639@gmail.com" class="fw-bold">vbansal639@gmail.com</a></div></div><div class="col-md-6"><div class="contact-card"><div class="icon"><i class="fas fa-handshake"></i></div><h4 class="h5">Partnerships & Media</h4><p class="text-muted">For partnership opportunities or media inquiries, please contact us at:</p><a href="mailto:vbansal639@gmail.com" class="fw-bold">vbansal639@gmail.com</a></div></div></div><div class="text-center mt-5"><h2 class="h3">Follow Us</h2><p class="text-muted">Stay connected with us on social media.</p><div class="contact-social-links mt-3"><a href="#" title="Twitter"><i class="fab fa-twitter"></i></a><a href="#" title="Facebook"><i class="fab fa-facebook-f"></i></a><a href="#" title="LinkedIn"><i class="fab fa-linkedin-in"></i></a><a href="#" title="Instagram"><i class="fab fa-instagram"></i></a></div></div></div></div>{% endblock %}"""
PRIVACY_POLICY_HTML_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}Privacy Policy - BrieflyAI{% endblock %}{% block content %}<div class="animate-fade-in"><div class="page-header-static"><h1>Privacy Policy</h1></div><div class="static-content-container"><p class="text-muted">Last updated: June 10, 2025</p><p>BrieflyAI ("we," "our," or "us") is committed to protecting your privacy. This Privacy Policy explains how we collect, use, disclose, and safeguard your information when you visit our website.</p><h2><i class="icon fas fa-shield-halved"></i>1. Information We Collect</h2><p>We may collect personal information that you voluntarily provide to us when you register on the website, post articles or comments, bookmark articles, or subscribe to our newsletter. This information may include your name, username, email address, and your activities on our platform such as articles posted and bookmarked.</p><h2><i class="icon fas fa-tasks"></i>2. How We Use Your Information</h2><p>We use the information we collect to:</p><ul><li>Create and manage your account.</li><li>Operate and maintain the website, including your profile page.</li><li>Display your posted and bookmarked articles as part of your profile.</li><li>Send you newsletters or promotional materials, if you have opted in.</li><li>Respond to your comments and inquiries.</li><li>Improve our website and services.</li></ul><h2><i class="icon fas fa-share-nodes"></i>3. Disclosure of Your Information</h2><p>Your username and posted articles are publicly visible. Your bookmarked articles are visible on your profile page to you when logged in. We do not sell, trade, or otherwise transfer your personally identifiable information like your email address to outside parties without your consent, except to trusted third parties who assist us in operating our website, so long as those parties agree to keep this information confidential.</p><h2><i class="icon fas fa-lock"></i>4. Security of Your Information</h2><p>We use administrative, technical, and physical security measures to help protect your personal information. While we have taken reasonable steps to secure the personal information you provide to us, please be aware that despite our efforts, no security measures are perfect or impenetrable.</p><h2><i class="icon fas fa-edit"></i>5. Your Choices</h2><p>You can review and change your profile information by logging into your account. You may also request deletion of your account and associated data by contacting us.</p><h2><i class="icon fas fa-sync-alt"></i>6. Changes to This Privacy Policy</h2><p>We may update this Privacy Policy from time to time. We will notify you of any changes by posting the new Privacy Policy on this page. You are advised to review this Privacy Policy periodically for any changes.</p></div></div>{% endblock %}"""
ERROR_404_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}404 Not Found{% endblock %}{% block content %}<div class='text-center my-5 p-4 article-card animate-fade-in mx-auto' style='max-width: 600px;'><h1><i class='fas fa-exclamation-triangle text-warning me-2'></i>404 - Page Not Found</h1><p class='lead'>Sorry, the page you are looking for does not exist or has been moved.</p><a href='{{url_for("index")}}' class='btn btn-primary mt-2'>Go to Homepage</a></div>{% endblock %}"""
ERROR_500_TEMPLATE = """{% extends "BASE_HTML_TEMPLATE" %}{% block title %}500 Server Error{% endblock %}{% block content %}<div class='text-center my-5 p-4 article-card animate-fade-in mx-auto' style='max-width: 600px;'><h1><i class='fas fa-cogs text-danger me-2'></i>500 - Internal Server Error</h1><p class='lead'>Something went wrong on our end. We've been notified and are looking into it.</p><a href='{{url_for("index")}}' class='btn btn-primary mt-2'>Go to Homepage</a></div>{% endblock %}"""

# --- End of Unchanged Templates ---
